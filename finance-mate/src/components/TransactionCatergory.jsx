...
function TransactionCatergory() {
    ...
    for (let i=0;i<all_trans.length;i++){
        all_categories.add(all_trans[i].personal_finance_category.primary);
    }
    ...
    all_trans.forEach((item)=>{
        cat_map_trans.set(item.personal_finance_category.primary,[...cat_map_trans.get(item.personal_finance_category.primary),item])
    })
  ...
  return (
    <div style={{height: "450px"}} className='dashboard' >
        <SideBar></SideBar>
